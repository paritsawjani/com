	...
	return Component.extend({
		...
		initObservable: function() {
			this._super().observe({
				...
				isAddressFormVisible:
					!customer.isLoggedIn() || addressOptions.length === 1,
			...
			quote.billingAddress.subscribe(function(newAddress) {
				if (quote.isVirtual()) {
					...
					this.isAddressSameAsShipping(
						newAddress != null &&
							newAddress.getCacheKey() ==
								quote.shippingAddress().getCacheKey() //eslint-disable-line eqeqeq
					);
				}
		...
		updateAddress: function() {
			...
			if (
				...
				this.selectedAddress() != newAddressOption
			) {
